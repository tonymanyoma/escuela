...
var controller = {
	...
	Login: (req, res) => {
		...
		connection_escuela.query('SELECT * FROM usuarios where usuario = ? AND password = ?', [usuario, password], (error, result) => {
	...
	Register: (req, res) => {
		...
		connection_escuela.query('SELECT * FROM usuarios where usuario = ? OR numero_documento = ? ', [usuario, numero_documento], (error, result) => {
			...
			if (result.length > 0) {
				...
				connection_escuela.query(sql, userObj, (error, result) => {
